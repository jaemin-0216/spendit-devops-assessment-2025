...
- Docker-compose는 사용하지 않게, RDS를 구성하도록 아키텍처를 구성했습니다.
- venv를 활용하여 이미지 빌드 속도, 불필요 빌드 도구 제거, 권한 문제를 해결하였습니다.

2. CI/CD 파이프라인 설계
.github/workflows/deploy.yml 참고 부탁 드립니다.


3. 설계 및 운영 계획서 작성
  a. 클라우드 아키텍처 다이어그램
    - 첨부한 Spendit.org 확인 부탁 드립니다.
  ...
  b. 기술 선택 및 설계 근거
    ...
    - 아키텍처 보안
      - DB 접속 정보는 Secret Manager에 등록하여 관리합니다. Secret Manager에 등록된 비밀번호는 passworkd rotation을 이용하여 특정 주기마다 변경되도록 합니다.
      - DB Data Encrypt는 RDS 기본 Amazon Managed Secret을 이용하여 원천 데이터에 대한 암호화를 합니다.
      - DB Data 암호화는 별도 Customer Managed Secret을 등록하여 요구조건에 맞는 암호화도 가능합니다.
  ...
  c. 운영 자동화 및 장애 대응 계획
    - 모니터링 전략:
      - RDS는 기본 CPU, Session에 대한 모니터링 지표를 제공하며 추가 옵션 활성화 시 Memory 등 추가 모니터링이 가능합니다.
      - lambda의 기본 메트릭은 아래와 같으며, 이
        Invocations (호출 수), Errors (에러 수), Duration (P50/P90/P99), Throttles (동시성 제한 발생), ConcurrentExecutions, Init Duration (Cold start 시간), DeadLetterErrors, IteratorAge / SQS batch age
      -  ERROR에 대한 알람을 받을 수 있도록 Cloudwatch Alarm을 활용하여 아키텍처를 구성 했습니다.
      - Cloudwatch Alarm 아키텍처는 Metic, SNS, Lambda Slack의 순서로 특정 장애 로그가 발생 하면 Metric으로 Cloudwatch Alarm을 발생 시켜 SNS에 토픽에 이를 적재하여 Lambda 함수를 실행, Slack 채널로 Alarm를 전송 시키는 구조를 구성했습니다. 
